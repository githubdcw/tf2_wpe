...
#from tensorflow.contrib import signal as tf.signal
...
def get_correlations(Y, inverse_power, taps, delay):
    ...
    Psi = tf.signal.frame(Y, taps, 1, axis=-1)[..., :T - delay - taps + 1, ::-1]
    Psi_conj_norm = (
...
def block_wpe_step(
    ...
    framed_Y = tf.signal.frame(
        Y, frames_per_block, frames_per_block, pad_end=True)
    framed_inverse_power = tf.signal.frame(
